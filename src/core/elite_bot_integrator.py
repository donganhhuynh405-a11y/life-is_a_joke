...
class EliteBotIntegrator:
    ...
    def validate_with_mtf(self, symbol: str, signal_direction: str):
        ...
        try:
            ...
            if confirmed:
                logger.info(
                    f"✅ MTF Analysis CONFIRMS {signal_direction.upper()} signal for {symbol} (Alignment: {trend_alignment:.2f}, Recommendation: {recommendation})")
            else:
                logger.warning(
                    f"❌ MTF Analysis REJECTS {signal_direction.upper()} signal for {symbol} (Alignment: {trend_alignment:.2f}, Recommendation: {recommendation})")
    ...
    def update_position_management(self, symbol: str, position: Dict,
        ...
        try:
            ...
            if updates:
                ...
                logger.info(
                    f"🎯 Advanced Position Management for {symbol}: Actions={updates.get('action')} SL={updates.get('stop_loss', 'N/A')} Partial={partial_tp.get('close_percentage', 0) * 100 if partial_tp else 0}%")
                return updates
