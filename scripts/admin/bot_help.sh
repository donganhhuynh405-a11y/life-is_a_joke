...
echo "--------------------------------------------------------------------------------"
echo -e "${CYAN}sudo /opt/trading-bot/scripts/update_bot.sh${NC}"
echo "   Update bot to latest version (recommended)"
echo ""
echo -e "${CYAN}cd /opt/trading-bot && git pull${NC}"
echo "   Pull latest changes from repository"
echo ""
echo -e "${CYAN}cd /opt/trading-bot && git status${NC}"
echo "   Check repository status and changes"
echo ""
echo -e "${CYAN}cd /opt/trading-bot && git log --oneline -10${NC}"
echo "   View last 10 commits"
echo ""

echo -e "${BLUE}🔍 DIAGNOSTICS & DEBUGGING${NC}"
echo "--------------------------------------------------------------------------------"
echo -e "${CYAN}python3 /opt/trading-bot/scripts/test_ai_system.py${NC}"
echo "   🤖 Test AI system (commentary, adaptive tactics, ML analyzers)"
echo ""
echo -e "${CYAN}python3 /opt/trading-bot/scripts/analyze_trades.py${NC}"
echo "   📊 Run full ML performance analysis"
echo ""
echo -e "${CYAN}python3 /opt/trading-bot/scripts/diagnose_positions.py${NC}"
...
echo ""
echo -e "${CYAN}python3 /opt/trading-bot/scripts/reset_daily_limit.py --status${NC}"
echo "   Check current daily trade limit status"
echo ""
echo -e "${CYAN}python3 /opt/trading-bot/scripts/reset_daily_limit.py${NC}"
...
echo ""
echo -e "${CYAN}sudo chmod +x /opt/trading-bot/scripts/*.sh${NC}"
...
echo "   sudo /opt/trading-bot/scripts/update_bot.sh"
echo ""
echo -e "${YELLOW}Test AI system:${NC}"
echo "   python3 /opt/trading-bot/scripts/test_ai_system.py"
echo ""
